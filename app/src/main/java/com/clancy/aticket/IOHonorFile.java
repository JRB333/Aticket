...
public class IOHonorFile {
	 ...
	 public static int HowManyPasses(String OptionalLine, Context cntxt)
	 {
		 String LineBuffer = "";
		 int ReturnSpaces = 0;
		 String deleteme1 = "";
		 ...
		 {
			 LineBuffer = SearchData.GetRecordNumberNoLength(WorkingStorage.GetCharVal(Defines.HonorFileNameVal, cntxt), 1, cntxt);
		 }
		 else
		 {
			 LineBuffer = SearchData.GetRecordNumberNoLength(OptionalLine, 1, cntxt);
			 WorkingStorage.StoreCharVal(Defines.HonorFileNameVal, OptionalLine, cntxt);
		 }
		 ...
		 deleteme1 = WorkingStorage.GetCharVal(Defines.SaveDateVal , cntxt);
	 ...
	 public static int ExpandHonorList(String WhatToWrite, Context cntxt)
	 {
	        int NumSpaces  = 0;
	        int NumStartSpace  = 0;
	        String ChrSpaces  = "";
	        ...
	        LotFileName = WorkingStorage.GetCharVal(Defines.HonorFileNameVal, cntxt);
	        
	        NumSpaces = Integer.valueOf(WorkingStorage.GetCharVal(Defines.SaveHBoxSpacesVal,cntxt).trim());
	        WorkingStorage.StoreLongVal(Defines.NumberOfSpacesVal, NumSpaces, cntxt);
	        
	        NumStartSpace = Integer.valueOf(WorkingStorage.GetCharVal(Defines.SaveHBoxEBirdTimeVal, cntxt).trim());
	        if (NumStartSpace < 1)
	        {
	        	NumStartSpace = 1;
	        }
	        WorkingStorage.StoreLongVal(Defines.NumStartSpaceVal, NumStartSpace, cntxt);
	        ...
	        for (int i = NumStartSpace; i <= (NumSpaces + NumStartSpace - 1); i++) 
	        {
	        	if (f == 0)
	        	{
	        		...
	        		f = 1;	 
	        		SaveToFile(LotFileName, WriteBuffer, cntxt);
	        	}
	        	WriteBuffer = WhatToWrite;
        		while (WriteBuffer.length() < 5)
                	WriteBuffer += " ";
	 ...
	 public static Boolean SaveToFile(String FileName, String WriteThis, Context cntxt)
	 {
	 ...
	 public static Boolean WriteVirtualFile(String cFileName, String cLineBuffer, int StartPos, Context cntxt)
	 {
		 Boolean CreatedOK = false;
