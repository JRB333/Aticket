...
public class NextCiteNum {
	...
	public static Boolean GetNextCitationNumber(int AddHowMany, Context cntxt)
	{
        String ClancyRecord  = "";
        ...
        WorkingStorage.StoreCharVal(Defines.PrintPrefixVal, PrintPrefix, cntxt);
        ...
        CitationValue = ValidateCheckDigit(PrintCitation, WhichCheckDigit, AddHowMany, cntxt);
        if (AddHowMany == 0)
        {
        	WorkingStorage.StoreCharVal(Defines.PrintCitationVal, CitationValue, cntxt);
        }
        else
        {
        	WorkingStorage.StoreCharVal(Defines.NextCitationVal, CitationValue, cntxt);
        }
        WorkingStorage.StoreCharVal(Defines.PrintLastCitationVal, ShowLastCitation, cntxt);
        
        return false;
	}

	public static String ValidateCheckDigit(String CitationPassed, String CDType, int AddHowMany, Context cntxt)
	{
        int NextCiteSeven  = 0;
        String NewSeven  = "";
        int NewDigit  = 0;
        ...
        if (WorkingStorage.GetCharVal(Defines.ClientName, cntxt).trim().equals("PGCOUNTY"))
        {
        ...
        if (WorkingStorage.GetCharVal(Defines.ClientName,cntxt).equals("VAIL"))
        {
        	NextCiteSeven = Integer.valueOf(CitationPassed.substring(0,7));
        	NextCiteSeven = NextCiteSeven + AddHowMany;
